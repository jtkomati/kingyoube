...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Creating IPVA payment for account:", accountHash);
    ...
    console.log("Sending IPVA request to TecnoSpeed");

    const response = await fetch(`${baseUrl}/payment/taxes/ipva`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      return new Response(
        JSON.stringify({ error: responseData.message || "Erro ao criar IPVA", details: responseData }),
    ...
    const { data: payment } = await serviceClient
      ...
      .insert({
        ...
        payment_form: "27",
        status: "CREATED",
        description: description || `IPVA ${vehiclePlates} - ${calculationYear}`,
        due_date: dueDate,
        payment_date: paymentDate,
        nominal_amount: nominalAmount,
        discount_amount: discountAmount,
        amount,
        beneficiary_name: `DETRAN ${state}`,
        beneficiary_cpf_cnpj: contributorDocument,
        tags,
        metadata: { tecnospeedResponse: responseData, revenueCode, vehiclePlates, vehicleRenavam, calculationYear }
      })
      .select()
      .single();
    ...
    console.log("IPVA payment created successfully:", uniqueId);
